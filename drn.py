...
class DRN(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000,
                 ...
                 out_map=False, out_middle=False, pool_size=28, arch='D', in_channels=3):
        ...
        if arch == 'C':
            self.conv1 = nn.Conv2d(in_channels, channels[0], kernel_size=7, stride=1,
    ...
    def load_state_dict(self, state_dict, resnet_pretrained=True):
        if resnet_pretrained is False:
            super().load_state_dict(state_dict)
            return
        first_layer_name, first_layer_weight = next(iter(state_dict.items()))
        mean_weight = torch.mean(first_layer_weight, dim=1, keepdim=True)
        new_first_layer_weight = torch.cat([first_layer_weight, mean_weight.repeat(1,self.in_channels-3,1,1)], dim=1)
        state_dict.update({first_layer_name: new_first_layer_weight})
        if self.num_classes != 1000:
            temp = state_dict.popitem()
            temp = state_dict.popitem()
