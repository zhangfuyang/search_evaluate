...
class Agent():
    ...
    def select_action(self, state, epsilon, network):
        model = network
        ...
        with torch.no_grad():
            ...
            next_candidates = candidate_enumerate_training(initial_candidate, 1, gt_data)
    ...
    def max_q_action(self, state, network):
        next_state = self.select_action(state, -1, network)
        return next_state
