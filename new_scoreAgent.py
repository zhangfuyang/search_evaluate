...
class UNet_big(nn.Module):
    def __init__(self, n_channels, n_classes, batchnorm, bilinear=False):
...
class UNnet(nn.Module):
    def __init__(self, batchnorm, corner_mask, bilinear=False, backbone_channel=64):
...
class scoreEvaluator_with_train(nn.Module):
    ...
    def __init__(self, datapath, backbone_channel=64, use_heat_map=True):
        super(scoreEvaluator_with_train, self).__init__()
        self.backbone_channel = backbone_channel
        self.backbone = UNet_big(3, backbone_channel, batchnorm=True) # for image
        #self.backbone2 = UNet_big(3, backbone_channel, batchnorm=False) # for image
        ...
        self.cornerNet = UNnet(batchnorm=True, corner_mask=True, backbone_channel=channel_size)
        self.edgeNet = UNnet(batchnorm=True, corner_mask=False, backbone_channel=channel_size)
        ...
        self.heatmapNet = UNet_big(3, 2, batchnorm=True)
    ...
    def get_score(self, candidate, all_edge=False, img_volume=None, heatmap=None):
        ...
        corner_map = corner_pred.squeeze().cpu().detach().numpy()#[0][0]
