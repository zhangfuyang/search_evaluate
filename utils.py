...
def get_edge_label(det_edge, corner_label, corner_assignment, gt_edges):
    edge_label = []
    for edge in det_edge:
        label = 0   
        endpoint_label = corner_label[edge]
        # both edge endpoints labelled as correct
        if np.sum(endpoint_label) == 2:
            gt_corner = corner_assignment[edge]
            if (gt_corner == gt_edges).all(axis=1).any():
                label = 1  # check if endpoint corners have gt edge or not
            if (gt_corner == np.fliplr(gt_edges)).all(axis=1).any():
                label = 1  # check if endpoint corners have gt edge or not
        edge_label.append(label)
    edge_label = np.array(edge_label) > 0.5
    return edge_label



def get_corner_label(gt_corner, det_corner, threshold):
    '''
    assign label to each detected corner based on distance to gt
    assignment is one-to-one preferring nearest distance first 
    '''
    # Compute Euclidean distance 
    dist_mat = np.zeros((det_corner.shape[0], gt_corner.shape[0]))  # N1 x N2
    for idx, corner in enumerate(det_corner):
        l2_dist = np.sqrt(np.sum(np.square(gt_corner - corner),1))
        dist_mat[idx,:] = l2_dist

    # One-to-one unique assignment 
    sorted_dist_mat = np.argsort(dist_mat, axis=1)  # increasing dist order
    mask = (np.sort(dist_mat, axis=1)<=threshold)[:,0]
    unique_ids = []
    corner_assignment = sorted_dist_mat[:,0]*mask
    for ii in np.where(mask==False)[0]:
        unique_id = random.randint(-2000, -2)
        while unique_id in unique_ids:
            unique_id = random.randint(-2000, -2)
        unique_ids.append(unique_id)
        corner_assignment[ii] = unique_id
    
    #np.copy(sorted_dist_mat[:,0])  # closest gt corner
    while(True):
        # check if two detected corner match to same gt corner
        repeat_gt_corners = [item for item, count in Counter(corner_assignment).items() if count > 1] 
        if len(repeat_gt_corners) == 0:
            break

        # break many-to-one matching, only assign gt corner to closest detected corner
        for repeat_gt_corner in repeat_gt_corners:
            # conflicting detected corners
            conflict_det_corners = np.where(corner_assignment==repeat_gt_corner)[0]
            current_dists = []
            next_dists = []
            for repeat_i in conflict_det_corners:
                # dist row for conflicting detected corners
                sorted_dist_row = sorted_dist_mat[repeat_i]  
                # find next closest assigned gt distance
                next_dist = sorted_dist_row[min(gt_corner.shape[0]-1, np.where(sorted_dist_row==repeat_gt_corner)[0][0]+1)]
                current_dist = dist_mat[repeat_i, repeat_gt_corner]
                current_dists.append(current_dist)
                next_dists.append(next_dist)

            # new assignment
            best_idx = np.argsort(current_dists)[0]
            for idx, repeat_i in enumerate(conflict_det_corners):
                if idx != best_idx:
                    if current_dists[idx] <= threshold:
                        corner_assignment[repeat_i] = next_dists[idx]
                    else:
                        # assign random negative unique id for larger than threshold det corner
                        unique_id = random.randint(-2000, -2)
                        while unique_id in unique_ids:
                            unique_id = random.randint(-2000, -2)
                        corner_assignment[repeat_i] = unique_id
                        unique_ids.append(unique_id)

    corner_assignment[corner_assignment<0] = -1
    corner_label = corner_assignment>=0
    return corner_label, corner_assignment
...
def get_wrong_corners(corners, gt_corners, edges, gt_edges):
    ...
    for gt_i in range(gt_corners.shape[0]):
        ...
        flag = True
        for id_ in sort_id:
            if dist_matrix[gt_i, id_] > threashold:
                break
    ...
    for gt_i in sorted(list(range(gt_corners.shape[0])), key=lambda i: np.min(dist_matrix[i])):
        sort_id = np.argsort(dist_matrix[gt_i]).__array__()[0]
        if dist_matrix[gt_i, sort_id[0]] > threashold:
            gt_match_location[gt_i] = None
        else:
            for c_i in sort_id:
                if c_i in matched:
                    continue
                if dist_matrix[gt_i, c_i] > threashold:
                    gt_match_location[gt_i] = None
                    break
                else:
    ...
    for edge_i in range(len(edges)):
        for edge_j in range(edge_i+1, len(edges)):
            ...
            if graph.isNeighbor(edgeA, edgeB):
                continue
            intersection_loc = get_two_edge_intersection_location(edgeA.x[0].x, edgeA.x[1].x, edgeB.x[0].x, edgeB.x[1].x)
...
region_cache = regionCache(os.path.join(config['data_folder'], 'result/corner_edge_region/entire_region_mask'))
