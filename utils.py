...
def draw_all(ax, img, corners, edges):
    ax.margins(0,0)
    ax.imshow(np.zeros_like(img)[:,:,0],cmap='gray', vmin=0, vmax=255)
    # plot line 
    XX = corners[edges][:,0,:]
    YY = corners[edges][:,1,:]
    for index, xx in enumerate(XX):
        ax.plot([xx[1], YY[index][1]], [xx[0], YY[index][0]], 'darkorange', linewidth=4)
    # plot corner
    #ax.plot(corners[:,1], corners[:,0], 'o', color='blue', markersize=2)
    ax.axis('off')
